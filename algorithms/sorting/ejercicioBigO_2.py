...
def check_if_lists_have_an_equal(list_a, list_b):  # O(n*m)
...
def print_10_or_less_elements(list_to_print):  # O(1)
	...
	for index in range(min(list_len, 10)):  # O(1) (loop runs at most 10 times, treated as constant)
		print(list_to_print[index])  # O(1)

def generate_list_trios(list_a, list_b, list_c):  # O(n*m*o)
	result_list = []  # O(1)
	for element_a in list_a:  # O(n)
		for element_b in list_b:  # O(m)
			for element_c in list_c:  # O(o)
